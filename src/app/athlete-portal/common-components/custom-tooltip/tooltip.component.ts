...
export class CustomTooltipComponent {
  ...
  get tooltipOffsetY(): number {
    return Number(this.data.options.offsetX);
  }
  ...
  setPosition(): void {
    ...
    const scrollY = getContainer().scrollTop;
    ...
    if (this.placement === 'top' || this.placement === 'bottom') {
      this.hostStyleLeft = (this.elementPosition.left + elementWidth / 2) - tooltipWidth / 2 + this.tooltipOffsetY + 'px';
    }
...
export function getContainer() {
  ...
  if (modalContainer) {
    ...
    return document.querySelector('[canvas="canvas-wrap"]');
  }
}